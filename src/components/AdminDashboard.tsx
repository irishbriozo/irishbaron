...
export default function AdminDashboard({
  ...
  const [adminTab, setAdminTab] = useState<'analytics' | 'orders' | 'reservations' | 'catering' | 'products' | 'announcements'>('orders');

  // Multi-choice state variables for adding product
  const [newProdName, setNewProdName] = useState('');
  const [newProdDesc, setNewProdDesc] = useState('');
  const [newProdPrice, setNewProdPrice] = useState(100);
  const [newProdCat, setNewProdCat] = useState<'hot-coffee' | 'iced-coffee' | 'kakanin' | 'bilao' | 'meals'>('meals');
      ...
      <div className="mx-auto max-w-md py-16 px-4 text-center space-y-6" id="unauthorized-access-banner">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-stone-500 leading-relaxed">
            This module contains GK Coffee operational dials. To view or execute status adjustments, please sign in using Chef Primo administrator credentials.
          </p>
        </div>
        <div className="pt-2">
          <button
  ...
  const handleAddProductSubmit = (e: React.FormEvent) => {
    ...
    setProductSuccess(`Success! "${newProdName}" added onto standard client menu.`);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8 pb-20" id="admin-dashboard-view-root">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-amber-100 pb-5 gap-4" id="admin-summary-header">
        <div>
          ...
          <h2 className="font-sans text-2xl font-extrabold text-stone-900 sm:text-3xl mt-1">Chef Primo&apos;s Command Center</h2>
          <p className="text-xs text-stone-500 mt-1">Analyze revenue stats, dispatch orders, approve dine-in bookings, and curate canteen menus.</p>
        </div>

        {/* Canteen operational metrics indicators */}
        <div className="flex gap-4" id="admin-tab-selection">
          <button
            onClick={() => setAdminTab('orders')}
            className={`px-3 py-2 text-xs font-semibold rounded-lg shrink-0 cursor-pointer ${
              adminTab === 'orders' ? 'bg-amber-800 text-white shadow-xs' : 'bg-amber-50 text-amber-900 hover:bg-amber-100'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-2 text-xs font-semibold rounded-lg shrink-0 cursor-pointer ${
              adminTab === 'reservations' ? 'bg-amber-800 text-white shadow-xs' : 'bg-amber-50 text-amber-900 hover:bg-amber-100'
            }`}
          >
            Tables ({pendingReservationsCount})
          </button>

          <button
            onClick={() => setAdminTab('catering')}
            className={`px-3 py-2 text-xs font-semibold rounded-lg shrink-0 cursor-pointer ${
              adminTab === 'catering' ? 'bg-amber-800 text-white shadow-xs' : 'bg-amber-50 text-amber-900 hover:bg-amber-100'
            }`}
          >
            Catering ({pendingCateringCount})
          </button>
          
          <button
            onClick={() => setAdminTab('products')}
            className={`px-3 py-2 text-xs font-semibold rounded-lg shrink-0 cursor-pointer ${
              adminTab === 'products' ? 'bg-amber-800 text-white shadow-xs' : 'bg-amber-50 text-amber-900 hover:bg-amber-100'
            }`}
          >
            Canteen Menu ({products.length})
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4" id="admin-kpi-grid">
        ...
        <div className="bg-white p-5 border border-amber-100/60 rounded-xl shadow-xs">
          <span className="text-[10px] text-stone-500 block uppercase font-bold tracking-wider">Table Reservations</span>
          <span className="text-xl font-extrabold text-amber-950 block mt-1">{pendingReservationsCount}</span>
          <span className="text-[10px] text-stone-500 mt-0.5 block">Waiting review and response</span>
        </div>
      ...
      <div className="flex border-b border-stone-100 justify-start gap-8" id="admin-subtabs">
        ...
        <button
          onClick={() => setAdminTab('reservations')}
          className={`pb-2.5 text-xs font-bold transition-colors cursor-pointer ${
            adminTab === 'reservations' ? 'text-amber-800 border-b-2 border-amber-800' : 'text-stone-400 hover:text-stone-700'
          }`}
        >
                ...
                >
                  ...
                  <div className="md:col-span-4 space-y-2">
                    ...
                    <div className="text-[10.5px] text-stone-500 leading-normal bg-stone-50/70 rounded-md p-2 mt-1">
                      ...
                      {order.type === 'delivery' ? (order.address || 'Address missing') : 'Pickup at Canteen Counter'}
                    </div>
                  </div>
      ...
      {/* ================= ADMIN VIEW 2: TABLE RESERVATIONS ================= */}
      {adminTab === 'reservations' && (
        <div className="space-y-6" id="admin-reservations-ledger">
          <h3 className="font-sans text-sm font-bold text-stone-900 uppercase tracking-widest pl-2 border-l-4 border-amber-800">
            Dine-In Space Reservation approvals
          </h3>

          <div className="overflow-x-auto rounded-xl border border-stone-200">
            <table className="w-full text-left text-xs text-stone-700 font-sans" id="reservations-table">
              <thead className="bg-stone-50 text-[10px] uppercase font-bold text-stone-500 border-b border-stone-200">
                <tr>
                  ...
                  <th className="px-4 py-3">Dining Date & Time</th>
                  <th className="px-4 py-3 text-center">Heads</th>
                  <th className="px-4 py-3">Requests / Notes</th>
              ...
              <tbody className="divide-y divide-stone-150">
                {reservations.length > 0 ? (
                  reservations.map((res) => (
                    <tr key={res.id} className="hover:bg-amber-50/20 transition-colors" id={`admin-res-row-${res.id}`}>
                      <td className="px-4 py-4.5 font-mono font-bold text-amber-950">{res.id}</td>
                      <td className="px-4 py-4.5">
                        <div className="font-semibold text-stone-900">{res.userName}</div>
                        <div className="text-[10px] text-stone-500 font-mono mt-0.5">{res.userEmail} | {res.phone}</div>
                      </td>
                      <td className="px-4 py-4.5">
                        <span className="block font-semibold text-stone-850">{res.date}</span>
                        <span className="block text-[10px] text-stone-500 font-mono mt-0.5">At {res.time}</span>
                      </td>
                      <td className="px-4 py-4.5 text-center font-bold">{res.guestsCount}</td>
                      <td className="px-4 py-4.5 max-w-xs truncate italic text-stone-500" title={res.notes}>
                        {res.notes || 'No requests'}
                      </td>
                      <td className="px-4 py-4.5">
                        <span className={`inline-block rounded px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${
                          res.status === 'pending' ? 'bg-yellow-50 text-yellow-950 border border-yellow-200' :
                          res.status === 'accepted' ? 'bg-emerald-50 text-emerald-950 border border-emerald-250' :
                          'bg-red-50 text-red-950 border border-red-200'
                        }`}>
                          {res.status}
                        </span>
                      </td>
                      <td className="px-4 py-4.5 text-right space-x-1">
                        {res.status === 'pending' && (
                          <>
                            <button
                              onClick={() => updateReservationStatus(res.id, 'accepted')}
                              className="rounded-lg bg-emerald-800 hover:bg-emerald-900 text-white font-bold p-1 border border-emerald-700 transition-colors inline-flex cursor-pointer"
                            ...
                            <button
                              ...
                              className="rounded-lg bg-stone-100 hover:bg-red-50 text-red-700 font-bold p-1 border border-red-200 transition-colors inline-flex cursor-pointer"
                  ...
                  <tr>
                    <td colSpan={7} className="text-center py-8 text-stone-500">
                      No customer reservations logged in memory ledger.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8" id="admin-inventory-ledger">
          ...
          <div className="lg:col-span-5" id="admin-add-product-panel">
            <div className="rounded-xl border border-amber-100 bg-amber-50/30 p-6 space-y-4">
              <div>
                <span className="font-mono text-[9.5px] font-extrabold uppercase tracking-widest text-amber-850">Canteen Operations</span>
                <h3 className="font-sans text-base font-bold text-amber-955 mt-0.5">Post New Available Food</h3>
                <p className="text-xs text-stone-500 mt-1">Populate drinks, meals, kakanin, or party packages instantly on the menu grid.</p>
              </div>
              ...
              <form onSubmit={handleAddProductSubmit} className="space-y-3.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <option value="kakanin">Kakanin Treats</option>
                      <option value="bilao">Bilao Packages</option>
                      <option value="meals">Hearty Meals</option>
                    </select>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  className="w-full rounded-lg bg-amber-800 hover:bg-amber-900 text-white font-bold py-2 text-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
